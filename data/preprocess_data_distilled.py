...
class Encoder(object):
    ...
    def encode(self, json_line):
        ...
        for key in self.args.json_keys:
            ...
            answer = data['answer']
            answer_start_pos = answer.find('<answer>')
            answer_end_pos = answer.find('</answer>')
            if answer_start_pos > -1 and answer_end_pos > -1:
                answer = answer[:answer_start_pos] + answer[answer_start_pos+len('<answer>'):answer_end_pos]
            conversation = [
                {'role':'user', 'content':question},
                {'role':'assistant', 'content':answer},
            ]
            sentence_ids = Encoder.tokenizer._tokenizer.apply_chat_template(conversation)
            sentence_lens = [len(sentence_ids)]
            if self.args.append_eod:
                sentence_ids.append(Encoder.tokenizer.eod)
                sentence_lens[-1] += 1
            ids[key] = sentence_ids
